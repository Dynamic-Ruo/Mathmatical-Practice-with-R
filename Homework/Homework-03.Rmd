...
gmp %>% ggplot() + geom_point(aes(x = pop, y = pcgmp)) + 
  geom_line(aes(x = pop, y = 6611 * pop ^ 0.125), col = 'blue', size = 1) + 
  geom_line(aes(x = pop, y = 6611 * pop ^ 0.10), col = 'red', size = 1) + 
  geom_line(aes(x = pop, y = 6611 * pop ^ 0.15), col = 'purple', size = 1) + 
...
```{r}
# mse is used to caluculate the mean squared erro of the model
mse <- function(parm, N = gmp$pop, Y = gmp$pcgmp){
  # compute the mean squared error
  mse <- mean((Y - parm[1] * N ^ parm[2]) ^ 2)
  return(mse)
}
```

```{r}
mse(c(6611, 0.15), gmp$pop, gmp$pcgmp)
# [1] 207057513
mse(c(5000, 0.10), gmp$pop, gmp$pcgmp)
# [1] 298459915
```
...
nlm(f = mse, p = c(y0 = 6611, a = 1 / 8), gmp$pop, gmp$pcgmp)
nlm(f = mse, p = c(y0 = 6611, a = 1 / 10), gmp$pop, gmp$pcgmp)
nlm(f = mse, p = c(y0 = 5000, a = 3 / 20), gmp$pop, gmp$pcgmp)
```
What do the quantities `minimum` and `estimate` represent? What values does it return for these?

'minimum' represents the mimimal value of mse function obtained in the iteration, and 'estimate' represents the value of parameters when mse achieves the minimum in the iteration.
...
```{r}
plm <- function(parm, N = gmp$pop, Y = gmp$pcgmp){
  # optimization
  nlm.result <- nlm(mse, c(y0 = parm[1], a = parm[2]), N, Y)
  ans <- list(estimate = nlm.result$estimate, MSE = nlm.result$minimum)
  return(ans)
}
```

```{r, message=FALSE, warning=FALSE}
plm(c(6611, 0.15), gmp$pop, gmp$pcgmp)
plm(c(5000, 0.10), gmp$pop, gmp$pcgmp)
```

We get y0 = 6611, a = 0.1263 and y0 = 5000, a = 0.1476 respectively. They are not the same because nlm() uses Newton's method to estimate the minimum of the function and this linear search method can only get local minimum.
...
```{r}
ans <- list(mean = mean(gmp$pcgmp), 
            sd_of_mean = sd(gmp$pcgmp) / (gmp$pcgmp %>% length %>% sqrt))
print(ans)
```
...
```{r}
n <- length(gmp$pcgmp)
jackknifed.means <- sapply(1:n, mean_omit_i)
```
...
The jack-knife approximation to the standard error of the mean is exactly the same as the answer from (a)
...
```{r}
plm.jackknife <- function(parm, N = gmp$pop, Y = gmp$pcgmp){
  # parameter estimate
  n = length(gmp$pcgmp)
  jackknife.estimate <- matrix(NA, ncol = 2, nrow = n) %>% as.data.frame
  colnames(jackknife.estimate) <- c('y0', 'a')
  for(omit in 1:n){
    jackknife.estimate[omit, ] <- plm(parm, N[-omit], Y[-omit])$estimate
  }
  
  # compute the standard error of the mean
  jackknife.sd <- sqrt((n - 1)^2 / n) * apply(jackknife.estimate, 2, sd)
  return(jackknife.sd)
}
```

```{r}
plm.jackknife(parm = c(6611, 0.15), N = gmp$pop, Y = gmp$pcgmp)
```
...
gmp2013 <- gmp2013 %>% mutate(pop = round(gmp / pcgmp))
# estimate
plm(c(6611, 0.15), gmp2013$pop, gmp2013$pcgmp)
plm.jackknife(c(6611, 0.15), gmp2013$pop, gmp2013$pcgmp)
```

The parameters of the model do not change significantly from the original one.
